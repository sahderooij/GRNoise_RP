...
def tau(freq, SPR,startf=None,stopf=None,decades=3,minfreq=1e2,plot=False,retfnl=False):
    '''Fits a Lorentzian to a PSD.
    Arguments:
    freq -- frequency array (in Hz)
    SPR -- power spectral denisty values (in dB)
    startf -- start frequency for the fit, default None: 3 decades lower than stopf, but bigger than 100 Hz
    stopf -- stop frequency for the fit, default the lowest value in the interval 3e2 to 3e4 Hz
    decades -- adjusts number of decades lower than stopf, if startf is None
    minfreq -- adjusts minimum frequency for startf, if startf is None
    ...
    if stopf is None:
        ...
        try:
            ...
            stopf = 25e4
    if startf is None:
        startf = max(10**(np.log10(stopf)-decades),minfreq)
